...
player.items.forEach(function func(item) {
  if(item.data.data.attributes.Type && item.data.data.attributes.Type.value === "Ranged" && item.data.data.attributes.Active.value === true){
...
if(weaponFound){
  ...
  const firingMessage = `<p>Firing <b>${weaponName}</b> (${timesToRoll})</p>`;
  ...
  promise.then(function (result) {
    ...
    promise.then(function (result) {
      const chatContent = firingMessage + result.data.content + totalMessage;
  ...
  const message = `<b>Please select your active ranged weapon</b>`;
